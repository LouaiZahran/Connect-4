...
class MinimaxWithoutPruning(Minimax):
    ...
    def _Minimax__min_function(self, state, maxDepth):
        ...
        return self.__min_function_util(state, -1, maxDepth,root), root

    def _Minimax__max_function(self, state, maxDepth):
        root=Node(self.__heuristic.get_score(state))
        return self.__max_function_util(state, -1, maxDepth,root), root
    ...
    def __min_function_util(self, state, best_index, maxDepth,root):
        ...
        for curr_state in states:
            ...
            v, i= self.__max_function_util(curr_state,best_index, maxDepth - 1,child)
            child.value=v
            if v < minV:
                minV = v
                best_index = i
    ...
    def __max_function_util(self, state, best_index, maxDepth,root):
        ...
        for curr_state in states:
            child = Node(-1)
            root.add_successor(child)
            v,i= self.__min_function_util(curr_state,best_index, maxDepth - 1,child)
            child.value=v
            if v > maxV:
                maxV = v
                best_index = i
